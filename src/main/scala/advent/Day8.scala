...
object Day8 {
  ...
  def part1(input: String): Int = {
    val quotedStrings = parseInput(input)
    val unquotedStrings = quotedStrings.map(decode)
    totalChars(quotedStrings) - totalChars(unquotedStrings)
  }

  private def parseInput(input: String): Seq[String] = input.lines.map(_.trim).toSeq

  def decode(quotedString: String): String = {

    def unescape(input: List[Char], output: String = ""): String = input match {
      case '\\' :: '\\' :: rest => unescape(rest, output + '\\')
      case '\\' :: '"' :: rest => unescape(rest, output + '"')
      case '\\' :: 'x' :: h1 :: h2 :: rest => unescape(rest, output + toChar(s"$h1$h2"))
      case regular :: rest => unescape(rest, output + regular)
      case Nil => output
    }

    def toChar(hexCode: String): Char = Integer.parseInt(hexCode, 16).toChar

    unescape(quotedString.substring(1, quotedString.length - 1).toCharArray.toList)
  }

  private def totalChars(strings: Seq[String]): Int = strings.map(_.length).sum

  def part2(input: String): Int = {
    val quotedStrings = parseInput(input)
    val doubleQuotedStrings = quotedStrings.map(encode)
    totalChars(doubleQuotedStrings) - totalChars(quotedStrings)
  }

  def encode(quotedString: String): String = {

    def escape(input: List[Char], output: String = ""): String = input match {
      case (metachar @ ('\\' | '"')) :: rest => escape(rest, output + '\\' + metachar)
      case regular :: rest => escape(rest, output + regular)
      case Nil => output
    }

    '"' + escape(quotedString.toCharArray.toList) + '"'
  }
